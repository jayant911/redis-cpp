...
// Fuunction to read line of text from the socket until it encounters a carriage
// return.
...
std::string ResponseParser::parseBulkString(int sockfd) {
    ...
    // Loop to read the bulk data from socket
...
std::string ResponseParser::parseArray(int sockfd) {
    ...
    for (int i = 0; i < count; ++i) {
        // recursively call the parseResponse for all the element in array.
